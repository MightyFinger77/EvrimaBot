...
**Bot on a different machine than the dedicated server:** The JAR can run on another PC or VPS. Set `rcon.host` to an address the bot can reach (LAN IP, hostname, or public IP)—**not** `127.0.0.1` unless the bot runs on the **same** machine as the game server. The dedicated server must accept RCON on an interface that path can reach (many setups bind RCON to localhost only); if you expose RCON across a network, use a strong password and tight firewall rules—RCON is full server control. **Discord** only needs outbound internet. Features that **read files** (`ingame_chat_log.path`, dino parking `playerdata_file.path_template`, etc.) use paths on **the host running the bot**; either mount or sync the real `TheIsle.log` / playerdata there (e.g. SMB/UNC, NFS), or disable those options if the bot cannot see the server’s files.
...
| **Duplicate slash commands** | Usually **global + guild** both registered, or an old `/evrima` tree plus the new split roots. **Preferred cleanup:** run **`remove-evrima.ps1 -BotToken "…"`** (or **`run-remove-evrima.bat`** with the same args) — it removes this app’s slash roots **globally and on every guild the bot is in**. Then restart the latest JAR with **`discord.guild_id`** set so registration is **guild-only** and predictable. On startup, with a real guild id, the bot also **clears global** commands for this application before registering on that guild; with `guild_id: 0` it clears on up to 25 guilds before registering globals. Portal → **Commands** is only needed if something else registered commands outside what the script removes. |
